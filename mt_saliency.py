...
def saliency(model, input_ids, decoder_input_ids, batch=0, correct=None, foil=None):
    ...
    decoder_input_ids = torch.tensor(decoder_input_ids, dtype=torch.long).to(
        model.device
    )
...
def erasure_scores(
    model, input_ids, decoder_input_ids, correct=None, foil=None, normalize=False
):
    model.eval()
    if correct is None:
        correct = input_ids[0][-1]
    ...
    for i in range(len(decoder_input_ids[0])):
        decoder_input_ids_i = torch.cat(
            (decoder_input_ids[0][:i], decoder_input_ids[0][i + 1 :])
        ).unsqueeze(0)
